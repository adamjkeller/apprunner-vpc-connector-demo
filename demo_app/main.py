...
def secrets_helper():
    ...
    secret_string = client.get_secret_value(SecretId=os.getenv("DBSECRETSNAME"))[
        "SecretString"
    ]
...
def ecs_private_service():
    ...
    return JSONResponse(
        status_code=status.HTTP_200_OK, content={"Response": str(result.content)}
    )
...
if __name__ == "__main__":
    ...
    uvicorn.run("main:app", host="0.0.0.0", port=8080, log_level="debug", debug=True)
